...
class StockResNet(nn.Module):
    def __init__(self, num_tickers: int, embedding_dim: int = 16) -> None:
        super().__init__()
        self.stem = nn.Sequential(
            nn.Conv1d(in_channels=3, out_channels=64, kernel_size=7, padding=3, bias=False),
            nn.BatchNorm1d(64),
            nn.ReLU(),
        )
        ...
        self.classifier = nn.Sequential(
            nn.Linear(128 + embedding_dim, 64),
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.Linear(64, 1),
        )
    ...
    def forward(self, x: torch.Tensor, ticker_id: torch.Tensor) -> torch.Tensor:
        price_features = self.resnet(self.stem(x)).squeeze(-1)
        ticker_features = self.ticker_embedding(ticker_id)
        combined = torch.cat([price_features, ticker_features], dim=1)
        return self.classifier(combined).squeeze(1)


def clean_resnet_frame(ticker: str, frame: pd.DataFrame) -> pd.DataFrame | None:
...
def make_resnet_window_data(
    ...
    for ticker, frame in sorted(frames.items()):
        ...
        clean_frame = clean_resnet_frame(ticker, frame)
    ...
    if not windows:
        raise ValueError("No ResNet windows were created from this split.")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a 1D ResNet on per-ticker stock CSV files.")
...
def main() -> None:
    ...
    train_windows = make_resnet_window_data(split_frames["train"], ticker_to_id)
    validation_windows = make_resnet_window_data(split_frames["validation"], ticker_to_id)
    test_windows = make_resnet_window_data(split_frames["test"], ticker_to_id)
    ...
    model = StockResNet(num_tickers=len(ticker_to_id)).to(device)
    print(
        f"Loaded {len(ticker_to_id)} ticker IDs. "
        f"ResNet windows train/validation/test: "
